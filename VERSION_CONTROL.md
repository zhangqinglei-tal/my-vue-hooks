# 版本控制功能说明

本项目已集成完整的版本控制和发布流程，支持将 hooks 打包为 SDK 进行发布。

## 📦 SDK 打包配置

### 构建输出

- **ES 模块**: `dist/my-vue-hooks.es.js` - 支持现代打包工具
- **CommonJS**: `dist/my-vue-hooks.cjs.js` - 支持 Node.js 环境
- **类型定义**: `dist/**/*.d.ts` - TypeScript 类型支持
- **Source Map**: 包含完整的源码映射，便于调试

### Tree Shaking 支持

- 使用 `preserveModules` 保持模块结构
- 支持按需导入，减少打包体积
- 每个 hook 都可以独立导入

### 版本信息注入

构建时自动注入版本号到代码中：

```ts
import { VERSION, getVersion } from 'my-vue-hooks'

console.log(VERSION)        // "1.0.0"
console.log(getVersion())    // "1.0.0"
```

## 🔢 版本管理

### 版本号规则

遵循 [语义化版本](https://semver.org/) 规范：

- **MAJOR** (主版本号): 不兼容的 API 修改
- **MINOR** (次版本号): 向下兼容的功能新增
- **PATCH** (修订号): 向下兼容的问题修正

### 版本管理脚本

#### 更新版本号

```bash
# 补丁版本 (1.0.0 -> 1.0.1)
pnpm version:patch

# 次版本 (1.0.0 -> 1.1.0)
pnpm version:minor

# 主版本 (1.0.0 -> 2.0.0)
pnpm version:major

# 直接指定版本号
node scripts/version.js 1.2.3
```

#### 自动发布流程

```bash
# 自动完成版本更新、测试、构建、CHANGELOG 更新
pnpm release:patch   # 或 :minor, :major
```

## 🚀 发布流程

### 自动发布（推荐）

```bash
# 一键发布补丁版本
pnpm release:patch

# 一键发布次版本
pnpm release:minor

# 一键发布主版本
pnpm release:major
```

发布脚本会自动执行：
1. ✅ 检查 Git 工作区是否干净
2. ✅ 运行所有测试
3. ✅ TypeScript 类型检查
4. ✅ 更新版本号（package.json）
5. ✅ 更新 CHANGELOG.md
6. ✅ 构建项目

### 手动发布步骤

1. **更新版本号**
   ```bash
   pnpm version:patch
   ```

2. **更新 CHANGELOG.md**
   编辑 `CHANGELOG.md`，添加新版本的变更记录

3. **构建项目**
   ```bash
   pnpm build
   ```

4. **运行测试**
   ```bash
   pnpm test:run
   ```

5. **提交更改**
   ```bash
   git add .
   git commit -m "chore: release v1.0.1"
   git tag v1.0.1
   ```

6. **推送到远程**
   ```bash
   git push && git push --tags
   ```

7. **发布到 npm**
   ```bash
   npm publish
   ```

## ✅ 发布前检查

在 `npm publish` 之前，会自动运行 `prepublishOnly` 脚本：

- ✅ 检查 `dist` 目录是否存在
- ✅ 检查必要的构建文件
- ✅ 运行测试套件
- ✅ TypeScript 类型检查
- ✅ 验证 package.json 配置

如果任何检查失败，发布将被阻止。

## 📝 CHANGELOG 管理

### 自动生成

使用 `pnpm release` 会自动在 `CHANGELOG.md` 中添加新版本条目。

### 手动编辑

遵循 [Keep a Changelog](https://keepachangelog.com/) 格式：

```markdown
## [1.0.1] - 2024-01-15

### Fixed
- 修复 useCounter 的边界值问题

### Changed
- 优化 useLocalStorage 的性能
```

## 🔧 配置文件

### package.json

- `version`: 当前版本号
- `files`: 发布时包含的文件（仅 `dist` 目录）
- `sideEffects`: 标记为无副作用，支持 tree shaking
- `exports`: 完整的导出映射，支持按需导入

### vite.config.ts

- 自动注入版本号到构建产物
- 配置 tree shaking 支持
- 生成 source map

### scripts/

- `version.js`: 版本号管理脚本
- `release.js`: 自动发布脚本
- `prepublish.js`: 发布前检查脚本

## 📊 版本信息使用

### 在代码中获取版本

```ts
import { VERSION, getVersion } from 'my-vue-hooks'

// 直接使用常量
console.log(VERSION)

// 使用函数
console.log(getVersion())
```

### 在构建产物中

构建后的文件会包含版本信息 banner：

```js
/*! my-vue-hooks v1.0.0 | MIT License */
```

## 🎯 最佳实践

1. **开发阶段**: 使用 `0.x.x` 版本号
2. **稳定版本**: 从 `1.0.0` 开始
3. **重大变更**: 升级主版本号
4. **新功能**: 升级次版本号
5. **Bug 修复**: 升级修订号
6. **发布前**: 始终运行 `pnpm release` 确保所有检查通过
7. **CHANGELOG**: 及时更新变更记录

## 🔍 验证发布

### 检查发布内容

```bash
# 查看将要发布的内容
npm pack --dry-run

# 打包但不发布
npm pack
```

### 查看已发布版本

```bash
# 查看所有版本
npm view my-vue-hooks versions

# 查看最新版本信息
npm view my-vue-hooks

# 查看特定版本
npm view my-vue-hooks@1.0.0
```

## 📚 相关文档

- [发布指南](./RELEASE.md) - 详细的发布流程说明
- [项目结构](./PROJECT_STRUCTURE.md) - 项目结构说明
- [贡献指南](./CONTRIBUTING.md) - 如何贡献代码

