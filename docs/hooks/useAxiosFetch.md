...
基于 Axios 的响应式数据请求 Hook，提供了完整的请求生命周期管理、错误处理、重试机制等功能。
...
const { data, loading } = useAxiosPost('/api/upload', formData, {
  responseType: 'form' // 可选，用于接收 FormData 响应
})

// 方式2：传入普通对象，自动转换为 FormData
const { data, loading } = useAxiosPost('/api/upload', {
  name: 'test',
  file: fileBlob
}, {
...
```ts
// Axios 版本
useAxiosGet<T>(url, options?)
useAxiosPost<T>(url, data?, options?)
useGetPromise<T>(url, options?)
usePostPromise<T>(url, data?, options?)
```
...
- [useFetch](/hooks/useFetch) - 基于原生 Fetch API 的数据请求 Hook
